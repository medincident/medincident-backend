...
-- identity in text form, which listeners use as the monotonic cursor for
-- their SELECT ... WHERE id > last_seen scan. NOTIFY is delivered at
-- transaction commit, so listeners only see events after the aggregate
-- write that produced them has already committed — preserving the
