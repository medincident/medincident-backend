...
func (s *IncidentTypeService) Reactivate(
	...
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		env, err := buildIncidentTypeReactivatedEnvelope(row.ID, updatedAt)
		if err != nil {
			return err
		}
		return outbox.Append(tx, "medincident.event.incident_type.v1.reactivated", env)
	})
	return ReactivateIncidentTypeResult{}, err
}

func buildIncidentTypeReactivatedEnvelope(typeID uuid.UUID, updatedAt time.Time) (*eventv1.Envelope, error) {
	msg := &classifierv1.IncidentTypeReactivated{TypeId: typeID.String(), UpdatedAt: timestamppb.New(updatedAt)}
